...
> Legacy-reference：本文件仅记录历史 App 接口接入矩阵。API 契约以后端 `C:/Users/qianlNya/RustroverProjects/rc0-rust/docs/openapi.yaml` 与真实 handler/service 代码为准；文档状态见 `docs/README.md`。
...
| 设备库（分类/品牌/机身/镜头/组合） | `EquipmentRepository` | `cine-equipment/api/cine-equipment-api.dart` → `listCineEquipmentBrands` / `listCineCameraBodies` / `listCineLenses` / `listCineCameraSetups` / `listMyCineCameraSetups` / `createCineCameraSetup` / `updateCineCameraSetup` / `deleteCineCameraSetup` / `listCineEquipmentFavorites` / `toggleCineEquipmentFavorite` | GET `/cine-equipment/brands?category&kind` · GET `/cine-equipment/bodies?category&brand` · GET `/cine-equipment/lenses?category&brand` · GET `/cine-equipment/setups?scope=0` · GET `/cine-equipment/setups/mine` · POST/PUT/DELETE `/cine-equipment/setups` · GET `/cine-equipment/favorites` · POST `/cine-equipment/favorites/toggle` |
| 制片资产（自定义分类 / 用户资产项） | `AssetRepository` | `production-assets/api/production-assets-api.dart` → `listProductionAssetCategories` / `createProductionAssetCategory` / `updateProductionAssetCategory` / `deleteProductionAssetCategory` / `listProductionAssetItems` / `createProductionAssetItem` / `updateProductionAssetItem` / `deleteProductionAssetItem` | GET/POST `/production-assets/categories` · PUT/DELETE `/production-assets/categories/{id}` · GET/POST `/production-assets/items` · PUT/DELETE `/production-assets/items/{id}` |
