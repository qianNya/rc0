...
| 分步同步树 | `ScreenplayPublishService._syncTreeStepwise` | `createAct/Scene/Frame` + PUT | POST/PUT `/screenplays/...` |
...
| 图片下载 URL | `DataUploadRepository._fetchDownloadUrl` | `getImageDownloadUrl` | GET `/images/{id}/download` |
...
| bulk `PUT /tree` | 改为分步 act/scene/frame CRUD + publish |
